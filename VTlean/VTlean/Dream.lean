...
def counterexample_C : Finset (List.Vector B 5) :=
  { ⟨[B.O, B.O, B.O, B.O, B.O], rfl⟩,
    ⟨[B.O, B.O, B.O, B.I, B.I], rfl⟩,
    ⟨[B.O, B.I, B.I, B.O, B.I], rfl⟩,
    ⟨[B.I, B.O, B.O, B.I, B.O], rfl⟩,
    ⟨[B.I, B.I, B.I, B.O, B.O], rfl⟩,
    ⟨[B.I, B.I, B.I, B.I, B.I], rfl⟩ }

theorem run_length_domination_false :
  ¬ (∀ {n : Nat} (C : Finset (List.Vector B n)) (hC : is_OptimalCodeCandidate C) (r : Nat),
    ∑ i ∈ Finset.Icc 1 r, num_words_with_runs C i ≤ ∑ i ∈ Finset.Icc 1 r, num_words_with_runs (Finset.VTCode n 0) i) := by {
  intro h
  have h_spec := h counterexample_C (by decide) 1
  revert h_spec
  decide
}
