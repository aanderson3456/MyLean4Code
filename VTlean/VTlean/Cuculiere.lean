...
/-- The roots of unity filter identity (Discrete Fourier Transform orthogonality).
    Expresses the sum of coefficients in residue class a as a character sum over the roots. -/
...
/-- Elementary real-complex inequality: multiplying a non-negative real by a complex number
    on the unit disc has a real part bounded by the real number itself. -/
lemma complex_re_im_le (z : ℂ) (hz : ‖z‖ ≤ 1) (y : ℂ) (hy_re : (0 : ℝ) ≤ y.re) (hy_im : y.im = 0) :
  ...
  have hz_re : z.re ≤ 1 := (Complex.re_le_norm z).trans hz
...
/-- The core theorem proving that the 0-th residue class is maximal in Cuculiere's triangle mod n+1.
    Proven using the roots of unity filter over Complex numbers, exactly as derived by Neil Sloane. -/
lemma cuculiere_mod_sum_gen_max (n : Nat) (a : Nat) :
  ...
  have h_sum_le : 
    ...
    have h_re_dist (s : Finset ℕ) (f : ℕ → ℂ) : (∑ x ∈ s, f x).re = ∑ x ∈ s, (f x).re := by {
      exact map_sum Complex.reAddGroupHom f s
    }
  ...
  have h_div_le :
    ...
    rw [h_factor, Complex.mul_re, Complex.mul_re]
