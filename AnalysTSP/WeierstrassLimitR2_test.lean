...
lemma CtsImagesCptRtoR2 {ι : Type} [Nonempty ι] {X : Set ℝ} {Y : Set (ℝ × ℝ)} (f : X → Y)
  ...
  · -- use continuity: dist (x_seq(φ n)) L < δ and x_seq(φ n) ≠ L
    ...
    -- Now need: euclideanDist (f(x_seq(φ n))).val (f L).val < ε
    -- The Lean dist on Y equals dist on ℝ×ℝ of the values (subtype inherits metric).
...
theorem PathsCompact {ι : Type} (S : Set (ℝ × ℝ)) : IsPathInR2 S → @IsCompactR2Subcover ι S := by {
  sorry
}
