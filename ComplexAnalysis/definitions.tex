...
def HasFDerivAt_R2_eps (u v : $\mathbb{R}$ $\times$ $\mathbb{R}$ $\to$ $\mathbb{R}$) (ux$_0$ uy$_0$ vx$_0$ vy$_0$ : $\mathbb{R}$) (a : $\mathbb{R}$ $\times$ $\mathbb{R}$) : Prop :=
  $\forall$ $\varepsilon$ > 0, $\exists$ $\delta$ > 0, $\forall$ x : $\mathbb{R}$ $\times$ $\mathbb{R}$,
    0 < euclideanDist x a $\wedge$ euclideanDist x a < $\delta$ $\to$
    euclideanNorm (u x - u a - (ux$_0$ * (x.1 - a.1) + uy$_0$ * (x.2 - a.2)), v x - v a - (vx$_0$ * (x.1 - a.1) + vy$_0$ * (x.2 - a.2))) < $\varepsilon$ * euclideanDist x a
\end{lstlisting}
...
/-!
  ...
  use Mathlib's `DifferentiableAt $\mathbb{R}$ f z` to represent continuous first partial derivatives / real-differentiability.
